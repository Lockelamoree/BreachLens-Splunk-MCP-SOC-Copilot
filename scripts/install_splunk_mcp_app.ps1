param(
    [string]$PackagePath = ".quarantine\splunk-mcp-server_026.tgz",
...
docker exec -e SPLUNK_PASSWORD="$password" -u splunk $Container sh -lc "/opt/splunk/bin/splunk install app '$remotePackage' -update 1 -auth '${SplunkUser}:$SPLUNK_PASSWORD'"
